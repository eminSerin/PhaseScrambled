function [] = checkMask()
%%
% checkMask checks how good masks fit to given images.
%
%
%
%% Import files

subID = input('participant number: '); % Write subject ID you want to check his/her images.
Imgpath  = [pwd, filesep, 'raw_images', filesep,num2str(subID), filesep];
allPics = dir([Imgpath, '*jpg']); % all pictures.

% Load images
for n = 1: length(allPics)
    rgbImages(:,:,:,n) = imread(allPics(n).name)*.95;
end
